...
"""Registration contract for the MySQL adapter.
...
from datus_mysql import MySQLConfig, MySQLConnector, register
...
def test_registration_exposes_current_adapter_contract():
    ...
    try:
        ...
        metadata = connector_registry.get_metadata("mysql")
        assert metadata is not None
        assert metadata.connector_class is MySQLConnector
        assert metadata.config_class is MySQLConfig
        # register() passes no display_name, so AdapterMetadata falls back to db_type.capitalize().
        assert metadata.display_name == "Mysql"
        # No parser_dialect override: sqlglot's dialect name already matches the db_type.
        assert metadata.parser_dialect is None

        # MySQL addresses objects as database.table -- it has neither a catalog nor a schema level.
        assert connector_registry.get_capabilities("mysql") == {"database"}
        assert connector_registry.support_database("mysql") is True
        assert connector_registry.support_catalog("mysql") is False
        assert connector_registry.support_schema("mysql") is False

        # datus-mysql ships no handlers.py/skills.py; the Agent's generic handling applies.
        assert connector_registry.get_uri_builder("mysql") is None
        assert connector_registry.get_context_resolver("mysql") is None
        assert connector_registry.get_identifier_parser("mysql") is None
        assert connector_registry.get_sql_generation_notes("mysql") is None
        # No factory: the registry constructs MySQLConnector(config) directly.
        assert "mysql" not in connector_registry._factories
    finally:
        _restore(saved)
...
def test_entry_point_is_discoverable_and_registers_under_its_own_name():
    ...
    saved = _snapshot()
    try:
        matches = [ep for ep in entry_points(group="datus.adapters") if ep.name == "mysql"]
        ...
        assert connector_registry.get_metadata(entry_point.name).connector_class is MySQLConnector
    finally:
        _restore(saved)
