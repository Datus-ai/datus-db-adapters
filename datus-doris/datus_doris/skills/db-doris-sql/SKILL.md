...
- `IPV4` and `IPV6` are storable column types. `VARBINARY` exists in Doris 4.0+ but **cannot be used in `CREATE TABLE`** — it is only reachable by mapping a binary column from an external catalog. Store binary payloads as `STRING`.
...
- The FE answers with a 307 redirect to a BE, so `--location-trusted` is required to carry credentials through it. When the BE address the FE returns is not routable from the client, post directly to a BE's HTTP port (8040 by default) using the same path.
