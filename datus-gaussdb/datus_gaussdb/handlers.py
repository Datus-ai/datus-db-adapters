...
"""Generic Agent integration hooks exposed by the GaussDB adapter."""
...
def build_gaussdb_uri(db_config) -> str:
    ...
    if not host:
        raise ValueError("GaussDB host is required")
    if not database:
        raise ValueError("GaussDB database is required")

    port = _config_value(db_config, "port") or "5432"
...
def resolve_gaussdb_context(db_config, uri: str) -> Tuple[str, str, str, str]:
    parsed = urlparse(uri)
    ...
    return "gaussdb", "", database, schema


def parse_gaussdb_identifier(full_table_name: str) -> Dict[str, str]:
    ...
    if not parts:
        return result
    if len(parts) > 3:
        raise ValueError(f"Invalid GaussDB table identifier: {full_table_name}")
...
def _split_identifier(identifier: str) -> list[str]:
    ...
    while index < len(text):
        ...
        if char in pairs:
            ...
            if not value:
                raise ValueError(f"Invalid GaussDB table identifier: {identifier}")
            parts.append(value)
            current = []
        else:
            current.append(char)
